...
//寻找邻居节点
  ...
  def senxdMsg(ET: EdgeTriplet[(VertexId, VertexState), Int]): Iterator[(VertexId, Array[Int])] = {
  ...
  def senxdMsg01(ET:EdgeTriplet[(VertexId, VertexState), Int] ):Iterator[(VertexId, Map[Int, Array[Int]])] = {
  ...
  def anonymity(resultGraph: Graph[(VertexId, VertexState), Int], sc: SparkContext, p:Int)={
    //将同一社区的节点分组
